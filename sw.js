...
// GitHub Pages variant: this repo serves the app at a fixed filename
// (index.html) rather than a build-numbered filename, so ONLY CACHE_NAME
// needs to be bumped on every new build — do NOT rename index.html itself,
// and don't mix this with the Termux/local variant (which keeps the build
// number in the HTML filename and bumps both the filename and CACHE_NAME
// together). The app forces an update check immediately (instead of
// waiting ~24h) and fetches the HTML/manifest network-first, so once you DO
// open the current URL, you'll never see stale content sitting behind an
// old cache.
const CACHE_NAME = 'gbels-v710';
