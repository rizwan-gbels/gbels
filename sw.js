...
// GitHub Pages variant: the HTML file is always served as index.html, so the
// filename never changes between builds — only bump CACHE_NAME (e.g.
// gbels-v701) each time you deploy a new build, so the service worker knows
// to fetch and cache the new version. (The Termux/local variant of this repo
// instead keeps the build number in the filename, e.g. GBELS_BUILD700.html —
// don't mix the two conventions, since it's the version that changed here,
// not the file being served.) What BUILD665 previously fixed still applies:
// the app forces an update check immediately (instead of waiting ~24h) and
// fetches the HTML/manifest network-first, so once you DO open the current
// URL, you'll never see stale content sitting behind an old cache.
const CACHE_NAME = 'gbels-v709';
